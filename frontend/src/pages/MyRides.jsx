...
export default function MyRides({ netid }) {
  ...
  const fetchMyRidesData = async () => {
    ...
    const endpoint = viewType === "posted" ? "/api/mypostedrides" : "/api/myrequestedrides";
    try {
        ...
        {myRidesData.length > 0 ? (
          myRidesData.map((ride) => (
            <RideCard
              ...
              handleRequestToJoin={(id) => console.log(`Requesting to join ride with ID: ${id}`)}
              handleCancelRequest={(id) => console.log(`Cancelling request for ride with ID: ${id}`)}
            />
          ))
        ) : (
